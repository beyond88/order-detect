<template>
  <div id="ordershield-sync-manage">
    ...
    <div class="sync-action-manage">
      ...
      <button
        v-if="syncInProgress"
        id="ordershield-order-stop-sync-btn"
        type="button"
        class="button"
        @click="stopSync"
      >
  ...
  methods: {
    async checkIfSyncPage() {
      ...
      if (
        ...
        params.get("tab") === "settings_tab_ordershield"
      ) {
    ...
    async pingSyncStatus() {
      try {
        ...
        const response = await axios.post(
          `${this.baseURL}/wp-json/ordershield/v1/sync-status`,
          {},
          {}
        );
    ...
    async startSync() {
      try {
        this.showSyncSpinner();
        const response = await axios.post(
          `${this.baseURL}/wp-json/ordershield/v1/start-sync`,
          {},
          {}
        );
    ...
    async stopSync() {
      try {
        ...
        const response = await axios.post(
          `${this.baseURL}/wp-json/ordershield/v1/stop-sync`,
          {},
          {}
        );
